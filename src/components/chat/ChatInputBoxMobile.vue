<template>
  ...
  <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
    ...
    <div v-if="showActionMenu" @click="showActionMenu = false" class="fixed inset-0 bg-black/30" style="z-index: 48;">
    </div>
  </transition>
  ...
  <div class="fixed bottom-0 left-0 right-0 z-50">
    ...
    <div class="bg-white border-t border-gray-200">
      <div class="flex items-center gap-2 p-3">
        <!-- 左侧：mic按钮 -->
        <div @click="showActionMenu = !showActionMenu"
        ...
        <div class="flex-1 bg-gray-100 rounded-xl px-4 py-2.5 flex items-center">
          <textarea ref="textareaRef" :value="modelValue"
            @input="$emit('update:modelValue', ($event.target as HTMLTextAreaElement).value)" @paste="handlePaste"
            @keydown="handleKeyDown" placeholder="iMessage" :disabled="disabled"
        ...
        <DropdownMenu v-model:open="showEmojiPicker">
          <DropdownMenuTrigger as-child>
            <div class="w-8 p-0 hover:bg-gray-100 flex-shrink-0 rounded-full">
              <Smile class="h-8 w-8 text-blue-500" />
            </div>
          </DropdownMenuTrigger>
          <DropdownMenuContent class="w-72 p-2" align="start" side="top">
            <div class="grid grid-cols-8 gap-1">
              <button v-for="(emoji, idx) in emojis" :key="idx" @click="insertEmoji(emoji)"
    ...
    <div v-if="showActionMenu" class="bg-white border-t border-gray-200 overflow-hidden">
      ...
      <div class="grid grid-cols-4 gap-4 p-4">
        <!-- 图片库 -->
        <button @click="imageInputRef?.click(); showActionMenu = false"
          ...
          <span class="text-xs text-gray-700 font-medium">图片</span>
        </button>

        <!-- 拍照 -->
        <button @click="cameraInputRef?.click(); showActionMenu = false"
          class="flex flex-col items-center gap-2 hover:opacity-80 transition-opacity">
        ...
        <!-- 文件 -->
        <button @click="fileInputRef?.click(); showActionMenu = false"
          class="flex flex-col items-center gap-2 hover:opacity-80 transition-opacity">
    ...
    <!-- 隐藏的文件输入框 -->
    <input ref="fileInputRef" type="file" class="hidden" @change="(e) => handleFileSelect(e, 'file')" />
